...
class QuestRepositoryImpl @Inject constructor(
    ...
    override fun observeQuest(questId: String): Flow<Quest?> =
        questDao.observeQuestWithObjectives(questId).map { it?.toDomain() }

    override suspend fun getQuest(questId: String): Quest? =
        questDao.getQuestWithObjectives(questId)?.toDomain()

    override suspend fun createQuest(spec: NewQuestSpec): String = db.withTransaction {
        val questId = newId()
        val ts = now()
    ...
    override suspend fun addProgress(
        ...
        return db.withTransaction {
            val entryId = newId()
            val ts = now()
            val row = questDao.insertProgressEntry(
    ...
    override suspend fun editProgress(entryId: String, newValue: Double): Boolean {
    ...
    override suspend fun completeQuest(questId: String): CompleteQuestResult {
        ...
        return db.withTransaction {
            ...
            val xpOutcome = progressionRepository.awardXp(
                userId = quest.userId, amount = xp.total,
                sourceType = XpSourceType.QUEST_COMPLETION, sourceId = questId,
            ...
            qwo.objectives.forEach { owe ->
                ...
                questDao.updateObjectiveProgress(
                    owe.objective.id, owe.objective.currentValue,
            ...
            qwo.objectives.forEach { owe ->
                val attribute = owe.objective.primaryAttribute
