...
class QuestFlowIntegrationTest {
    ...
    fun setUp() {
        ...
        progression = ProgressionRepositoryImpl(
            db, db.playerDao(), db.xpDao(), db.attributeDao(), LevelCalculator(), RankCalculator(),
        )
        quests = QuestRepositoryImpl(
            db, db.questDao(), progression, XpCalculator(), AttributeProgressCalculator(),
        )
        runBlocking {
    ...
    private suspend fun create200PushUpQuest(): Pair<String, String> {
        val questId = quests.createQuest(
            NewQuestSpec(
                ...
                objectives = listOf(
                    NewObjectiveSpec(
                        title = "Push-ups", type = ObjectiveType.REPETITIONS, target = 200.0, unit = "reps",
    ...
    @Test
    fun `full 200 push-up flow awards xp once and grows strength and discipline`() = runTest {
    ...
    @Test
    fun `imported duplicate progress does not double count`() = runTest {
        val (questId, objectiveId) = create200PushUpQuest()
        val first = quests.addProgress(
            objectiveId, 50.0, source = com.ascend.core.model.ProgressSource.HEALTH_CONNECT,
            sourceApplication = "com.example.health", externalRecordId = "rec-1",
        )
        val dup = quests.addProgress(
            objectiveId, 50.0, source = com.ascend.core.model.ProgressSource.HEALTH_CONNECT,
            sourceApplication = "com.example.health", externalRecordId = "rec-1",
        )
