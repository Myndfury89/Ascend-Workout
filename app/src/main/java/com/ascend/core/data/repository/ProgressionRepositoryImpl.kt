...
class ProgressionRepositoryImpl @Inject constructor(
    ...
    override suspend fun getProgress(userId: String): PlayerProgress? =
        playerDao.getProgress(userId)?.toDomain(levelCalculator)
    ...
    override suspend fun awardXp(
        ...
        val row = xpDao.insertIgnoringDuplicates(
            XpTransactionEntity(
                id = newId(), userId = userId, amount = amount, transactionType = "AWARD",
                sourceType = sourceType.name, sourceId = sourceId, description = description,
                createdAt = now(),
            ),
        )
    ...
    override suspend fun reverseXp(
        ...
        val original = xpDao.find("AWARD", sourceType.name, sourceId)
            ...
            if (amount != 0L) {
                attributeDao.insertIgnoringDuplicates(
                    AttributeTransactionEntity(
                        id = newId(), userId = userId, attributeType = attribute.name, amount = amount,
    ...
    private suspend fun recomputeProgress(
        ...
        val entity = (before ?: PlayerProgressEntity(userId = userId, updatedAt = now())).copy(
            level = state.level,
    ...
    private suspend fun recomputeStats(userId: String) {
        ...
        val entity = (current ?: PlayerStatsEntity(userId = userId, updatedAt = now())).copy(
