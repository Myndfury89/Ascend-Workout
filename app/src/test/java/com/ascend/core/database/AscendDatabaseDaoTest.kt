...
class AscendDatabaseDaoTest {
    ...
    @Test
    fun `duplicate xp award for same source is blocked but reversal is allowed`() = runTest {
        val award = XpTransactionEntity(
            id = "x1", userId = userId, amount = 350, transactionType = "AWARD",
            sourceType = "QUEST_COMPLETION", sourceId = "q1", description = "Quest done", createdAt = 1,
        )
        ...
        val reversal = XpTransactionEntity(
    ...
    @Test
    fun `accumulation sums uneven sets and recomputes after delete`() = runTest {
        seedPushUpQuest()
    ...
    @Test
    fun `duplicate imported record is blocked while manual entries are not`() = runTest {
        seedPushUpQuest()

        val imported = QuestProgressEntryEntity(
        ...
        val m1 = questDao.insertProgressEntry(
            QuestProgressEntryEntity(
                id = "m1", questId = "quest1", objectiveId = "obj1", value = 10.0,
                source = "MANUAL", completedAt = 11, createdAt = 11, updatedAt = 11,
            ),
        )
        val m2 = questDao.insertProgressEntry(
            QuestProgressEntryEntity(
                id = "m2", questId = "quest1", objectiveId = "obj1", value = 10.0,
                source = "MANUAL", completedAt = 12, createdAt = 12, updatedAt = 12,
            ),
        )
    ...
    private suspend fun seedPushUpQuest() {
        questDao.upsertQuest(
            QuestEntity(
                id = "quest1", userId = userId, title = "200 Push-Ups", questType = "ACCUMULATION",
                baseRewardXp = 350, createdAt = 0, updatedAt = 0,
            ),
        )
        questDao.upsertObjective(
            QuestObjectiveEntity(
                id = "obj1", questId = "quest1", title = "Push-ups", objectiveType = "REPETITIONS",
                targetValue = 200.0, unit = "reps", preferredSetSize = 25,
            ),
        )
    }

    private suspend fun insertSet(id: String, value: Double) {
        questDao.insertProgressEntry(
            QuestProgressEntryEntity(
                id = id, questId = "quest1", objectiveId = "obj1", value = value,
                source = "MANUAL", completedAt = value.toLong(), createdAt = 0, updatedAt = 0,
            ),
        )
    }
}
