...
/** The dark internal gradient fill — slightly uneven, brighter toward the upper-centre sigil. */
    ...
    drawPath(
        ...
        brush =
            Brush.radialGradient(
                colors =
                    listOf(
                        Color(0xFF161C28).copy(alpha = materialize),
                        Color(0xFF0C1017).copy(alpha = materialize),
                        Color(0xFF080B11).copy(alpha = materialize),
                    ),
    ...
    drawPath(
        ...
        brush =
            Brush.verticalGradient(
                colors = listOf(accent.copy(alpha = 0.05f * materialize), Color.Transparent),
                startY = 0f,
...
/** The luminous edge: thin overall, brighter at the two cut corners; drawn as the outline stroke. */
fun DrawScope.drawStatusPanelEdge(
    accent: Color,
    materialize: Float,
) {
    val path = panelPath(size)
    drawPath(path, color = accent.copy(alpha = 0.20f * materialize), style = Stroke(width = 1.dp.toPx()))
    drawPath(path, color = accent.copy(alpha = 0.55f * materialize), style = Stroke(width = 2.dp.toPx()))
    // Brighter breaks at the clipped corners.
    val bright = accent.copy(alpha = 0.9f * materialize)
